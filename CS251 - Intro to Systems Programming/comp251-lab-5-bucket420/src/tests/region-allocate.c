 ...
 * region.
 *
 * This test only verifies that the number of pages/regions increase. Another
 * test validates that regions are cleaned up.
 *
...
int main(int argc, char **argv) {
  ...
  EXPECT_LT(t.pages[0], t.pages[2], "Expect pages remain allocated");
  EXPECT_LT(t.regions[0], t.regions[1],
            "Expect regions allocated after malloc");
  EXPECT_LT(t.regions[0], t.regions[2],
            "Expect regions allocated after malloc");
